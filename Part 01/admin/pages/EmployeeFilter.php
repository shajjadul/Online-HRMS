...
document.getElementById('department').disabled = false}
      ...
      function AttenFilter()
      {
        ...
        if(document.getElementsByName("filter")[1].checked && document.getElementById('department').selectedIndex == 0)
        {
        alert("Select department.");
        document.forms['AttendenceFilter']['department'].style.background = 'Yellow';
        return false;
        }		  
...
<?php
	...
	if($filter==1)
	{
	$result=mysql_query("select * from add_employee where status='2' ORDER BY emp_joiningdate ASC");
	}
	else if($filter==2)
	{
	$result=mysql_query("select * from add_employee where emp_department='$department' and (status='0' or status='1') ORDER BY emp_joiningdate ASC");
	}
	else if($filter==3)
	{
	$result=mysql_query("select * from add_employee where emp_id='$EmpId' and (status='0' or status='1')");
	}
	
?>

<form name="AttendenceFilter" action="EmployeeFilterTemp.php?ac=employee" method="post" onsubmit="return AttenFilter()">
<div style="padding:3px 0 5px 100px; margin-top:10px; background-color:#D1D1D1; font-size:18px;">
   ...
   <div style="float:left; padding-left:10px;"><input type="radio" name="filter" value="1" id="CurMonth"/> Terminate</div>
   <div style="float:left; padding-left:10px;">
   <input type="radio" name="filter" value="2" id="PreMonth"/> Department 
    ...
    <div class="Table">
        <div class="Title">
			...
			<?php
                if($filter==1)
                {
                    echo "Tarminated employee List of Gateway Group";
                }
                else if($filter==2)
                {
                    echo "Employee List of ".$department_array[$department];
                }
                else if($filter==3)
                {
                    echo "Curriculum vitae of ".$EmpId;
                }
            ?>
        </div>
    <div class="Heading">
...
?>   
<div class="content">
   <div style="width:980px; min-height:450px; margin:2px 8px 10px 8px; background-color: #CEE7FF;"><!--Page Area -->
   <div>
         
 <div align="center" style="width:980px; min-height:158px; border-bottom:1px solid #000;">
 <div>
 ...
 <div style="min-height:200px; width:930px; margin:5px 0 0 50px;">
  <table><tbody>
  <tr>
  <th id="sub_td">Name</th> <td width="3%">:</td> <td><?php echo $rows01[2];?></td>
  </tr>
  <tr>
 ...
 <div style="background:#C68C8C; height:32px; margin-top:5px;"><h3 style="color:#00F; margin:0 0 0 20px;">SCHOLASTIC STATUS</h3></div>
        ...
        <div style="margin:10px 0 10px 10px; width:960px; min-height:150px;">
          ...
          <div style="margin:5px 0 10px 50px;  width:930px; min-height:15px;">
          <?php echo $rows04[2];?>
          </div>
   ...
   if($row[8]==2)
   {
	$result99=mysql_query("select * from emp_termination where EmpId ='$row[2]'");
	$field99=mysql_fetch_array($result99);
	
     list($ter_y,$ter_m,$ter_d)=explode("-",$field99[3]);
	 $ter_month=$month_array[$ter_m];
	 $ter_day=$ter_d." ".$ter_month." ".$ter_y;
   ?>
   <div style="min-height:25px; border:2px solid red; padding:5px 10px 5px 10px; font-size:18px;">
     <div><b>Termination date:</b> <?php echo $ter_day;?></div>
     <div><b>Reason:</b> <?php echo $field99[2];?></div>
   </div>
   <br />
   <?php
   }
...
?>
        <div class="content">
        <div style="height:25px; width:996px; color:#FF0000; font-weight:800; font-size:18px; text-align:center; background-color:#CEE7FF;">This profile is not completed by the employee.</div>
        </div>
<?php
		}
		}
